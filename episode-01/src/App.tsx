...
const generateWeekData = (startDate: Date): WeekData => {
	...
	const mockHours = [8, 7.5, 8, 6, 8, 0, 0]
	const mockProjects = [
		"Client A - Web Redesign",
		"Client A - Web Redesign",
		"Internal - Team Meetings",
	...
	const mockNotes = [
		...
		"Weekly standup and retrospective",
			...
			<div className="relative p-5">
				...
				<div className="mb-4">
					...
					<ProjectSelector
						value={entry.project}
						onChange={(value) =>
							onUpdate(entry.id, "project", value)
						}
						disabled={disabled}
					/>
				</div>
...
function App() {
	...
	const [weekData, setWeekData] = useState<WeekData>(
		generateWeekData(getMonday(new Date()))
	)
	...
	const navigateWeek = useCallback(
		(direction: "prev" | "next") => {
			...
			// Update week data with navigation
			setWeekData((prevData) => {
				const newWeekStart = new Date(currentWeek)
				newWeekStart.setDate(
					currentWeek.getDate() + (direction === "next" ? 7 : -7)
				)
				return {
					...generateWeekData(newWeekStart),
					...
					<div className="pt-6 border-t border-white/10">
						...
						<div className="space-y-2">
							{[
								...
								{ status: "submitted" as const, count: 1 },
								{ status: "approved" as const, count: 8 },
							].map((item) => {
