			...
			for (const entry of weekData.entries) {
				if (entry.hours > 0 && entry.projectId) {
					const entryData = {
						...
						cha_hours: entry.hours.toString(),
						cha_notes: entry.notes || undefined,
						"cha_Project@odata.bind": `/cha_projectses(${entry.projectId})`,
					}
					...
					if (entry.timesheetEntryId) {
						...
						// Create new entry
						await Cha_timesheetentriesService.create(entryData as any)
					}
				}
			}

			setWeekData((prev) => ({ ...prev, status: "submitted" }))
			confetti({
				particleCount: 100,
				spread: 70,
				origin: { y: 0.6 },
