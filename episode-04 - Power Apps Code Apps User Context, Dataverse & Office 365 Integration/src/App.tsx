		...
		const searchUsers = async () => {
			...
			try {
				...
				if (result.data?.value) {
					setUsers(
						result.data.value.map((u: Office365UsersModel.User) => ({
							id: u.Id,
	...
	return (
		<div className="relative">
			<button
				...
				disabled={disabled}
				className="relative w-full cursor-pointer rounded-xl bg-white/5 border border-white/10 py-2.5 pl-4 pr-10 text-left text-white/90 text-sm focus:outline-none focus:ring-2 focus:ring-indigo-500/50 transition-all duration-200 hover:bg-white/10 disabled:opacity-50 disabled:cursor-not-allowed"
			>
			...
			<AnimatePresence>
				{isOpen && (
					<motion.div
						...
						exit={{ opacity: 0, y: -10 }}
						className="absolute z-20 mt-2 w-full rounded-xl bg-slate-800/95 backdrop-blur-xl border border-white/10 shadow-2xl overflow-hidden"
					>
						{/* Search Input */}
						<div className="p-2 border-b border-white/10">
							<div className="relative">
								...
								<input
									...
									className="w-full rounded-lg bg-white/5 border border-white/10 py-2 pl-9 pr-4 text-white text-sm placeholder:text-white/40 focus:outline-none focus:ring-2 focus:ring-indigo-500/50"
									autoFocus
								/>
							</div>
						</div>
								...
								users.map((user) => (
									<button
										...
										className={`w-full relative cursor-pointer select-none py-2.5 pl-10 pr-4 text-left text-sm transition-colors hover:bg-indigo-500/20 ${
											value?.id === user.id ? "text-white bg-indigo-500/10" : "text-white/70"
										}`}
									>
										<div className="flex flex-col">
											<span className={`block truncate ${value?.id === user.id ? "font-semibold" : ""}`}>
												{user.displayName}
											</span>
...
const ProjectSelector = ({
	...
	return (
		<Listbox value={value} onChange={handleChange} disabled={disabled}>
			<div className="relative">
				<Listbox.Button className="relative w-full cursor-pointer rounded-xl bg-white/5 border border-white/10 py-2.5 pl-4 pr-10 text-left text-white/90 text-sm focus:outline-none focus:ring-2 focus:ring-indigo-500/50 transition-all duration-200 hover:bg-white/10 disabled:opacity-50 disabled:cursor-not-allowed">
					<span className="block truncate">
				...
				<Transition
					...
					leaveTo="opacity-0"
				>
					<Listbox.Options className="absolute z-20 mt-2 max-h-60 w-full overflow-auto rounded-xl bg-slate-800/95 backdrop-blur-xl border border-white/10 py-2 shadow-2xl focus:outline-none">
		...
		<motion.div
			...
			className={`group relative overflow-hidden rounded-2xl backdrop-blur-xl border transition-all duration-300 ${
				isToday
					...
					: "bg-white/5 border-white/10 hover:border-white/20 hover:shadow-xl hover:shadow-indigo-500/5"
			}`}
		>
			...
			<div className="relative p-5">
				...
				<div className="mb-4">
					...
					<div className="relative">
						<input
							...
							disabled={disabled}
							className="w-full rounded-xl bg-white/5 border border-white/10 py-2.5 px-4 text-white text-sm focus:outline-none focus:ring-2 focus:ring-indigo-500/50 transition-all duration-200 hover:bg-white/10 disabled:opacity-50 disabled:cursor-not-allowed [appearance:textfield] [&::-webkit-outer-spin-button]:appearance-none [&::-webkit-inner-spin-button]:appearance-none"
						/>
						<Clock className="absolute right-3 top-1/2 -translate-y-1/2 w-4 h-4 text-white/30" />
					</div>
				</div>
				...
				<div className="mb-4">
					...
					<textarea
						...
						className="w-full rounded-xl bg-white/5 border border-white/10 py-2.5 px-4 text-white text-sm placeholder:text-white/30 focus:outline-none focus:ring-2 focus:ring-indigo-500/50 transition-all duration-200 hover:bg-white/10 resize-none disabled:opacity-50 disabled:cursor-not-allowed"
					/>
				</div>

				{/* Daily Total */}
				<div className="flex items-center justify-between pt-3 border-t border-white/10">
					<span className="text-sm text-white/50">Daily Total</span>
					<span
						className={`text-lg font-bold ${
							entry.hours > 0 ? "text-white" : "text-white/30"
						}`}
					>
						{entry.hours}h
					</span>
...
function App() {
	...
	const [timesheets, setTimesheets] = useState<Cha_timesheetsModel.Cha_timesheets[]>([])
	...
	useEffect(() => {
		const loadTimesheetData = async () => {
			...
			try {
				...
				const projectsData: Project[] = projectsResult.data?.map((p) => ({
					id: p.cha_projectid,
					name: p.cha_name,
				})) || []
				...
				const entries: DayEntry[] = days.map((day, i) => {
					...
					return {
						...
						day,
						date,
						project: matchedProject?.name || matchingEntry?.cha_projectname || "",
						projectId: projectId,
						hours: matchingEntry?.cha_hours ? parseFloat(matchingEntry.cha_hours) : 0,
						notes: matchingEntry?.cha_notes || "",
						timesheetEntryId: matchingEntry?.cha_timesheetentryid || null,
					}
				})
				...
				if (matchingTimesheet?.owneridname && matchingTimesheet?._owninguser_value) {
	...
	const projectBreakdown = weekData.entries.reduce((acc, entry) => {
	...
	const handleSubmit = async () => {
		try {
			...
			if (timesheetId) {
				...
				const updateResult = await Cha_timesheetsService.update(timesheetId, timesheetData)
				console.log("Timesheet update result:", updateResult)
			} else {
				// Create new timesheet
				console.log("Creating new timesheet...")
				const result = await Cha_timesheetsService.create(timesheetData as any)
			...
			for (const entry of weekData.entries) {
				console.log(`Entry: ${entry.day}, hours: ${entry.hours}, projectId: ${entry.projectId}`)
				if (entry.hours > 0 && entry.projectId) {
					...
					if (entry.timesheetEntryId) {
						...
						console.log("Updating existing entry:", entry.timesheetEntryId)
						const updateResult = await Cha_timesheetentriesService.update(
						...
						const createResult = await Cha_timesheetentriesService.create(entryData as any)
			...
			setWeekData((prev) => ({ ...prev, status: "submitted", timesheetId }))
			confetti({
				particleCount: 100,
				spread: 70,
				origin: { y: 0.6 },
	...
	return (
		<div className="min-h-screen bg-linear-to-br from-slate-900 via-slate-800 to-slate-900">
			...
			{/* Sidebar */}
			<aside
				className={`fixed top-0 left-0 h-full w-64 bg-slate-900/95 backdrop-blur-xl border-r border-white/5 z-50 transform transition-transform duration-300 ease-out lg:translate-x-0 ${
				...
				<div className="flex flex-col h-full p-6">
						...
						{[
							...
							<motion.button
								...
								className={`w-full flex items-center gap-3 px-4 py-3 rounded-xl text-left transition-all duration-200 ${
									item.active
										? "bg-linear-to-r from-indigo-500/20 to-purple-500/20 text-white border border-indigo-500/30"
					...
					<div className="pt-6 border-t border-white/10">
						...
						<div className="space-y-2">
							{[
								{ status: "draft" as const, count: timesheets.length || 1 },
			...
			<div className="lg:pl-64">
				...
				<header className="sticky top-0 z-30 bg-slate-900/80 backdrop-blur-xl border-b border-white/5">
					<div className="flex items-center justify-between px-4 lg:px-8 h-16">
						...
						<div className="flex items-center gap-2">
							...
							<div className="px-4 py-2 rounded-xl bg-white/5 border border-white/10 min-w-45 text-center">
				...
				<div className="px-4 lg:px-8 py-4">
					<div className="bg-linear-to-r from-indigo-500/20 via-purple-500/20 to-indigo-500/20 rounded-2xl border border-white/10 p-4">
				...
				<div className="px-4 lg:px-8 mb-4 flex flex-col sm:flex-row items-start sm:items-center gap-4">
					<div
						className={`inline-flex items-center gap-2 px-4 py-2 rounded-xl ${statusConfig.bg} border border-white/10`}
					>
						<StatusIcon
					...
					{loading ? (
						<div className="flex items-center justify-center py-20">
							<div className="flex flex-col items-center gap-4">
								...
								<p className="text-white/70">Loading timesheet data...</p>
							</div>
						</div>
					) : (
				...
				{/* Footer Summary */}
				<footer className="sticky bottom-0 bg-slate-900/95 backdrop-blur-xl border-t border-white/10">
					<div className="px-4 lg:px-8 py-4">
								...
								{Object.entries(projectBreakdown).map(
									([project, hours]) => (
										...
										>
											...
											<span className="text-sm text-white/70 truncate max-w-37.5">
												{project}
											</span>
